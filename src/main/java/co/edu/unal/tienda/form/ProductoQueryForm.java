...
public class ProductoQueryForm {
	private static final Logger LOG = Logger.getLogger(ProductoQueryForm.class.getName());
     ...
     */
    public ProductoQueryForm filter(Filter filter) {
        if (filter.operator.isInequalityFilter()) {
     ...
     */
    @ApiResourceProperty(ignored = AnnotationBoolean.TRUE)
    public Query<Producto> getQuery() {
        // First check the feasibility of inequality filters.
        checkFilters();
        Query<Producto> query = ofy().load().type(Producto.class);
